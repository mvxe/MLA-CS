...
void RPBBSerial::serial_ack(std::vector<uint32_t> &commands, unsigned ackqueue, unsigned num, const char breakChar){
        ...
        for(int j=0;j!=num;j++){
            ...
            for(int i=0;i!=8;i++){
                ...
                if(breakOnLF) {
                    commands.push_back(CQF::IF_GPIO((breakChar&(1<<i))?CQF::LOW:CQF::HIGH,false,_gpioN_i,_gpioP_i));
                    commands.push_back(CQF::FLAGS_LOCAL_SET(0x0000));
                    commands.push_back(CQF::END());
                }
                commands.push_back(CQF::WAIT(1*_cyclesPerBit-(breakOnLF?5:2)));            // wait until approx the middle of next bit
            }
        }
...
void RPBBSerial::string_to_command(std::string string, std::vector<uint32_t> &commands, uint16_t W4FLAGS_SHARED){
    ...
    for (int i=0;i!=string.length();i++){
        ...
        //commands.push_back(CQF::W4TRIG_MIN_IN_QUEUE(15));
        commands.push_back(CQF::W4TRIG_MIN_IN_QUEUE(1+2+8*2+2));
