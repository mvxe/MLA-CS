...
namespace imgfor{
    ...
    inline img_form ocv_type_get(VmbPixelFormatType vmb_type){
        for (int i=0;i!=sizev;i++) if(convmat[i].vmb_type==vmb_type) return convmat[i];
        return {vmb_type,-1,"none",(cv::ColorConversionCodes)-1};
    }
}
