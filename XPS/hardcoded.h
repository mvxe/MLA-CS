...
public:
    ...
    enum GroupID { mgroup_XYZ, mgroup_GR1, mgroup_GR2 };   // <-- <-- <-- <-- <-- <-- <-- put the ID of each new group here, this is used to reference it throughout the code, first enum is always 0, DO NOT REARANGE THEM TO KEEP CONFIG FILES BACKWARD COMPATIBLE!
    struct group{
        const GroupID ID;
        const int AxisNum;
